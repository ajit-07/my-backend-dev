...
const getParticularBooks = async function (req, res) {
    let found = req.body
    let allBooks = await BookModel.find(found)
    res.send({ msg: allBooks })
}
const getXINRBooks = async function (req, res) {
    let allBooks = await BookModel.find({ $or: [{ "prices.indianPrice": "Rs.100" }, { "prices.indianPrice": "Rs.200" }, { "prices.indianPrice": "Rs.500" }] })
    res.send({ msg: allBooks })
}
